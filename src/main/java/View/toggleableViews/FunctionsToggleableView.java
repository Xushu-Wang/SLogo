...
public class FunctionsToggleableView extends ToggleableView {
  ...
  public FunctionsToggleableView(int width, int height, String language) {
    super.setResources(language);
    super.setView(makeView());
    super.makeButton("FunctionsButton", getFromResources("Functions"));
  ...
  public void updateView(String context, String info) {
    if (! context.equals("Function") || info.equals("")) {
      return;
    }
    myView.getChildren().clear();
    for (String variable : info.split("ENDCOMMAND")) {
